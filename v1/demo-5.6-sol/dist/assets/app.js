...
function createActors() {
  const avatar = createAnimatedActor(state.avatarId, .92);
  player = { ...avatar, position: new THREE.Vector3(0, 0, 12), velocity: new THREE.Vector3() };
  ...
  animals = wildlife.map((definition, index) => {
    const actor = createAnimatedActor(definition.model.replace("animal-", "").replace(".glb", ""), definition.id === "bee" ? .65 : .9);
    ...
    actor.object.userData.interaction = { type: "wildlife", actor };
  ...
  npcActors = npcs.map((definition) => {
    const actor = definition.kind === "animal" ? createAnimatedActor(definition.modelId, .92) : createHumanActor(definition);
    actor.object.position.set(definition.position.x, 0, definition.position.z);
    actor.definition = definition;
    actor.object.userData.interaction = { type: "npc", actor };
...
function updatePlayer(delta) {
  ...
  if (keyboardMoving) {
    ...
    moveTarget = pendingInteraction.actor.object.position.clone();
    moveTarget.y = 0;
  }
  if (!keyboardMoving && moveTarget) {
    const toTarget = moveTarget.clone().sub(player.object.position);
    toTarget.y = 0;
    const stopDistance = pendingInteraction ? (pendingInteraction.type === "npc" ? 2.45 : 2.15) : .16;
    if (toTarget.length() <= stopDistance) {
      ...
      if (pendingInteraction) {
        ...
        else openDiscovery(interaction.actor);
      }
    } else {
  ...
  if (moving) {
    if (keyboardMoving) input.normalize().applyAxisAngle(new THREE.Vector3(0, 1, 0), cameraYaw);
    ...
    if (!isBlocked(next)) {
      ...
      const left = input.clone().applyAxisAngle(new THREE.Vector3(0, 1, 0), Math.PI / 3);
      const right = input.clone().applyAxisAngle(new THREE.Vector3(0, 1, 0), -Math.PI / 3);
      const leftStep = player.object.position.clone().addScaledVector(left, delta * 3.2);
      const rightStep = player.object.position.clone().addScaledVector(right, delta * 3.2);
      if (!isBlocked(leftStep)) input.copy(left), player.object.position.copy(leftStep);
      else if (!isBlocked(rightStep)) input.copy(right), player.object.position.copy(rightStep);
      else {
        moveTarget = null;
        pendingInteraction = null;
        showToast("前面被樹叢擋住了，試試另一個落腳點。");
      }
    }
...
function openDiscovery(actor) {
  ...
  if (wasNew) {
    showToast(state.discoveredIds.size === wildlife.length ? "手札集齊了。森林仍然不趕時間，繼續走走吧。" : `手札新增：${item.name}`);
  }
}
...
function handleWorldClick(clientX, clientY) {
  if (!raycaster || !camera || !player || state.phase !== "explore") return;
  ...
  const actorObjects = [...npcActors, ...animals].map((actor) => actor.object);
  const actorHit = raycaster.intersectObjects(actorObjects, true).map((hit) => interactionFromObject(hit.object)).find(Boolean);
  if (actorHit) {
    pendingInteraction = actorHit;
    moveTarget = actorHit.actor.object.position.clone();
    showToast(actorHit.type === "npc" ? `走近${actorHit.actor.definition.name}聊聊。` : `走近看看${actorHit.actor.definition.name}。`);
    return;
  }
  const destination = new THREE.Vector3();
  if (!raycaster.ray.intersectPlane(groundPlane, destination)) return;
  destination.y = 0;
  if (isBlocked(destination)) {
    showToast("那裡不方便落腳，點旁邊的空地試試。");
    return;
  }
  pendingInteraction = null;
...
function bindEvents() {
  ...
  dom.canvas.addEventListener("pointerup", (event) => {
    if (!pointerMoved) handleWorldClick(event.clientX, event.clientY);
    isDragging = false;
